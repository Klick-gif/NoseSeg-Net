...
def choose_model(config):
    if config.model == 'unet':
        ...
        return UNetPlusPlus(in_channels=config.in_channels, out_channels=config.num_classes)
    else:
...
class UNetPlusPlus(nn.Module):
    def __init__(self, in_channels=1, out_channels=2):
        ...
        # 额外的嵌套连接（UNet++ 风格，维度已验算，安全）
        self.up4_extra = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.dec4_extra = DoubleConv(512, 256)
    ...
    def forward(self, x):
        ...
        # 额外的嵌套连接（UNet++ 风格）
        d4_extra = self.up4_extra(d4)
        d4_extra = torch.cat([d4_extra, e3], dim=1)
        d4_extra = self.dec4_extra(d4_extra)
